...
def generar_fecha_aleatoria():
    while True:
        mes, dia = random.choice(FECHAS_FESTIVAS)
        fecha = fake.date_time_between(start_date='-30d', end_date='now')
        fecha = fecha.replace(month=mes, day=dia)
        return fecha
...
def generar_datos_aleatorios_con_fraude(num_pedidos, nombres_compradores, numeros_cuenta, id_generados):
    ...
    rangos_cantidad_productos = {nombre: (random.randint(25, 35), random.randint(100, 120)) for nombre in nombres_compradores}
    rangos_cantidad_precio = {nombre: (random.randint(20, 45), random.randint(100, 180)) for nombre in nombres_compradores}
    
    with open("ArchivosAdicionales/datos_pedidos_fraude_Esc3.txt", "w") as archivo:
        for _ in range(num_pedidos):
            ...
            if random.random() < 0.05:
                ...
                if tipo_fraude == 'cambio_direccion':
                    ...
                    cantidad_productos = random.choice([random.randint(1, int(min_cantidad * 0.5)), int(max_cantidad * random.choice([1.8, 2.0]))])
                    ...
                    costo_pedido = random.choice([random.randint(1, int(min_cantidadPrecio*0.5)), int(max_cantidadPrecio * random.choice([1.8, 2.0]))])
                ...
                historial_cliente = historial_cambios_direccion.get(nombre_apellido, [])
                cambios_mes_actual = sum(1 for fecha in historial_cliente if fecha >= datetime.now() - timedelta(days=30))
                if cambios_mes_actual > 10 and tipo_fraude == 'No_Fraude':
